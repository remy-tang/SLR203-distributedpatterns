	...
	public void onReceive(Object message) throws Throwable {
		if(message instanceof ActorRef){
			...
			for (int i=0; i<5; i++) {
				...
				log.info("["+getSelf().path().name()+"] sent request to ["+ refB +"]: ID " 
				...
				log.info("["+getSelf().path().name()+"] received response from [b]: ID " 
						+ ((ResponseMessage)resp).getId() + ", message " + ((ResponseMessage)resp).getMessage());
			}
		}
	}
}